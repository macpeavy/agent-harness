...
**1. Sandbox: a container, not a microVM or full VM.** Run `opencode serve` and the substrate (ADR 0004's processes 2 and 3) inside a single long-lived **container** (Podman preferred — rootless, daemonless, ports to both Linux and macOS; Docker acceptable). Non-root user, read-only root filesystem where possible, only the target repo bind-mounted, no host credential mounts (`~/.ssh`, `~/.aws`, host `auth.json`/`mcp-auth.json` never visible inside). The container is the unit of isolation, chosen over a microVM (KVM-only, does not port to the required macOS target) and a full VM (ops weight unjustified for a solo operator). The threat is exfiltration by an injected agent, which egress control + credential isolation + filesystem scoping fully address; it is not a kernel-escape adversary. A **single long-lived** container preserves ADR 0004's persistent client-server topology — it is not the per-task sandbox model 0004 rejected; "direct-host" there meant "not a fresh sandbox per build," and one durable sandbox honors that.
...
**3. Default-deny network egress.** The sandbox drops all outbound traffic except an explicit allowlist (iptables/nftables default-DROP, or a filtering proxy). The allowlist is the minimum the loop needs:
   ...
   Everything else, including `webfetch`/`websearch` destinations, is dropped at the network layer regardless of OpenCode config. This single control neutralizes `bash` curl/nc, `webfetch`, rogue MCP, and plugin sockets at once.
...
- Podman vs Docker for the sandbox runtime, and host-process vs sibling-container for LiteLLM — implementation choices the build settles; both honor this ADR.
