...
- **Schema as code is the source of truth.** Tables are defined in TypeScript (`src/substrate/schema.ts`); `drizzle-kit generate` (`bun run db:generate`) emits SQL migrations into `drizzle/` (committed — migrations are source). The registry applies pending migrations at startup (`migrate()` in the constructor), so a fresh db self-initializes and a restart picks up new migrations. This answers 0009's flagged schema-versioning open question.
