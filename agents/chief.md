...
- **`decompose`** — pass 2: write one **session's chunk-DAG** (its chunks + dependency edges). Call it per session, after `meta_decompose`. *Decomposing is the design; the builder does the typing.*
...
- **Revise before approval** — while the feature is in `planning`, edit the plan on the owner's feedback: `revise_chunk` (re-spec a chunk), `remove_chunk` / `remove_session` / `remove_edge` (prune). Iterate until they're satisfied, then `dispatch` (approve). Frozen once approved.
