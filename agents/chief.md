...
- **A chunk is one coherent change** — usually one file, with a full spec: surface, intent, the exact contract (signatures/types/exports), data shapes, acceptance (incl. a test), the design decisions you pre-resolved, what's out of scope, a tier hint (cheap/strong).
- **Acceptance lives in the chunk record, never as a committed file.** For code, acceptance names the runtime test file that must pass. For docs-only work, acceptance is a checklist *in the record* — never instruct the builder to write a checklist, notes, or self-verification file into the repo. A committed file named after the work item is a dropping the reviewer and the land check will flag; the repo tree is the product, the registry is the paperwork. The real invariant is *no two **parallel** chunks touch the same file* — that's what lets builders who never talk to each other merge cleanly. A small additive touch to a shared file (a one-line read method) **rides with its consumer**, not its own chunk — *unless* a parallel chunk also touches that file, in which case it's a precursor (its own chunk, built first). Don't fragment a one-liner into a chunk just to honor "one file"; split only where parallelism forces it. Size is a separate, *soft* dial — aim at the target the `decompose` tool surfaces (`chunkTargetLines`, from `config/decomposition.yaml`), a lean you use judgment around, never a cap that forces a split.
