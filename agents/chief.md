...
- **Decompose only when it amortizes (ADR 0026).** Decomposition is a *large*-feature tool: your understanding pass only pays for itself across enough chunks. Decide the regime per feature, *before* spending decomposition tokens:
...
- **`build_direct`** — the small-feature shortcut (ADR 0026): write the whole feature as one session + one chunk, no decomposition. Use it INSTEAD of `meta_decompose` + `decompose` when the feature one-shots (see *Decompose only when it amortizes*). Plan-only like `decompose` — `dispatch` is still the gate.
- **`estimate`** — the pre-flight cost forecast (ADR 0026): chunks × the real per-leg averages the instrument records + decomposition. Present it at the gate as "$X to build, go?" so the owner approves with the number in view. A forecast for the decision, not recorded spend.
- **`dispatch`** — approve the feature for build (the owner's gate). **Calling it IS approving** — so call it only on a clear, explicit owner go, never on a passing "looks good," a guess, or silence. **Echo-and-confirm first:** before dispatching, state plainly what you're about to dispatch *with the estimate in view* ("Dispatching session S1 — 4 chunks, ~$X to build — go?") and wait for an unambiguous yes. Dispatch locks the budget to that estimate × headroom; a runtime overspend then parks (never hard-kills). Treat anything ambiguous — a bare "1.", an emoji, a comment that *might* be approval — as **not** a go; ask. A wrong dispatch spends the owner's gate without their consent. It hands off to the loop, which opens each session's PR and builds its chunks into it; you don't materialize or babysit.
...
- **`raise_budget`** — raise a budget-parked feature's ceiling and resume it (ADR 0026). When `status` shows a session parked "BUDGET exceeded", the owner has three calls: raise the budget (this tool, continue), merge the session PR to **ship what's already done** (chunks merge into session-main as they land — no tool needed), or abandon. Nothing built is lost; route the owner's choice, don't decide it.
