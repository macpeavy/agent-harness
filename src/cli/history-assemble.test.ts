...
import type { FeatureGraph } from "../substrate/plan";
import type { Chunk, Edge, Feature, Session } from "../substrate/plan";
...
describe("assembleHistory", () => {
  ...
  it("sums totalCostUsd correctly", () => {
    ...
    expect(result.sessions[0].totalCostUsd).toBe(0.45);
  });
  ...
  it("handles dispatch with null costs as zero", () => {
    ...
    const dispatch = makeDispatch("dispatch-1", "chunk-1", {
      buildCostUsd: 0.1,
      reviewCostUsd: undefined,
      amendCostUsd: undefined,
    });
    chunk.dispatchId = "dispatch-1";
